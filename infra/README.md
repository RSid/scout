...
| `docker-compose.prod.yml` | Host-neutral prod stack: `app` (runtime image) + `db`, with profile-gated ingest services. |
| `start.sh` | Production entrypoint — runs Alembic, then uvicorn + Next + Caddy side-by-side. |
