...
| `scripts/ingest_dc_points_of_interest.py` (DEC-026) | Paginates the OCTO MAR "Points of Interest - MAR Aliases" ArcGIS layer (or reads `data/dc_points_of_interest.jsonl`), joins each `MAR_ID` against the already-ingested `dc_addresses` table for coordinates/street address, upserts into `dc_points_of_interest` (idempotent). Supports `--fetch`, `--write-jsonl`, `--dry-run`, `--database-url`, `--log-level`. Requires `dc_addresses` to be populated first. | After `make ingest-dc-addresses` + `make migrate`; refresh with `--fetch --write-jsonl data/dc_points_of_interest.jsonl` before releases. Prefer `make ingest-dc-pois` for local loads. |
| `scripts/ingest_dc_street_segments.py` (DEC-027) | Paginates the OCTO/DDOT `DDOT_TOPS` Street Centerline (SubBlock) ArcGIS layer (or reads `data/dc_street_segments.jsonl`), upserts LineString segments into `dc_street_segments` (idempotent), then stamps each `features` row's nearest `street_name` via a KNN `UPDATE`. Supports `--fetch`, `--write-jsonl`, `--dry-run`, `--database-url`, `--log-level`. Requires `features` to be populated first (`make ingest-write`). | After `make ingest-write` + `make migrate`; refresh with `--fetch --write-jsonl data/dc_street_segments.jsonl` before releases. Prefer `make ingest-dc-street-segments` for local loads. |
...
| `scripts/fetch_map_glyphs.sh` (DEC-028) | Downloads the Latin + General-Punctuation `Noto Sans Regular` SDF glyph PBFs (+ OFL license) from protomaps/basemaps-assets into `apps/web/public/fonts/glyphs/`. Self-hosted per NF-PRIV-01; the basemap style serves them same-origin at `/fonts/glyphs/{fontstack}/{range}.pbf`. Idempotent. | Before the first interactive `pnpm dev` on a fresh clone (map street labels, M2-F25), or after changing the label fontstack. |
