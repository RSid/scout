...
| `scripts/ingest_dc.py`     | Idempotent DC OpenData (+ optional OSM Overpass amenities) → PostGIS `features` ingestion (M1-F11 / M1-T03). Flags: `--dry-run`, `--include-osm`/`--no-include-osm`, `--database-url`, `--log-level`. Whole ingest runs in one transaction; reruns with unchanged inputs report `inserted=0 updated=0 unchanged=N`. | Standing up the local DB (`make ingest` for a dry-run; the Compose `ingest` profile for a real write). |
