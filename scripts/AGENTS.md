...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Makefile (repo root)       | Shortcut targets for sync, lint, Compose, DC feature ingest (`make ingest` runs `scripts/ingest_dc.py --dry-run` when present), ingest dry-run (`make help`). Prefer over ad-hoc tool invocations. | Everyday dev + agent workflows; aligns with DEC-011.                                 |
