...
type PersistedPreferences = Readonly<{ version: 1; selections: Record<string, boolean> }>
...
export function ProfileProvider({
  ...
  useEffect(() => {
    hydrateFromCategories().catch(() => {
      const stored =
        typeof window !== 'undefined' ? readSelections() : null
  ...
  const resetToDefaults = useCallback(() => {
    setSelections(mergeDefaults(categories.length > 0 ? categories : SAMPLE_CATEGORIES_FALLBACK, null))
  }, [categories])
